...
router.post('/currency/convert', async (req, res) => {
  try {
    ...
    const conversion = await currencyService.convertAmount(amount, from_currency, to_currency);
    res.json({ conversion });
    ...
    res.status(500).json({
      ...
      message: error.message || 'Internal server error during currency conversion'
    });
  }
});

// Get exchange rate
router.get('/currency/rate/:from/:to', async (req, res) => {
  try {
